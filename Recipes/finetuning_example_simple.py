...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume, use_wandb, wandb_resume_id, gpu_count):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join(MODELS_DIR, "mos_ToucanTTS")  # RENAME TO SOMETHING MEANINGFUL FOR YOUR DATA
    os.makedirs(save_dir, exist_ok=True)

    train_data = prepare_tts_corpus(transcript_dict=moore_audio_metadata_to_dict(),
                                    corpus_dir=os.path.join(PREPROCESSING_DIR, "integration_test"),
                                    lang="mos")  # CHANGE THE TRANSCRIPT DICT, THE NAME OF THE CACHE DIRECTORY AND THE LANGUAGE TO YOUR NEEDS
    ...
    train_loop(net=model,
               ...
               eval_lang="mos",  # THE LANGUAGE YOUR PROGRESS PLOTS WILL BE MADE IN
               warmup_steps=500,
